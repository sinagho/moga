...
def build_norm_layer(norm_type, embed_dims):
    ...
    if norm_type == 'LN2d':
        return LayerNorm(embed_dims, eps=1e-6)
    if norm_type == 'SyncBN':
...
class LayerNorm(nn.Module):
    ...
    def forward(self, x):
        if self.data_format == "channels_last":
            ...
            # Compute mean and variance along the (H, W, D) dimensions for each channel
            u = x.mean(dim=(2, 3, 4), keepdim=True)
            s = (x - u).pow(2).mean(dim=(2, 3, 4), keepdim=True)
            x = (x - u) / torch.sqrt(s + self.eps)
            # Adjust weight and bias to match the number of channels and expand for spatial dims
            x = self.weight[:, None, None, None] * x + self.bias[:, None, None, None]
            return x


class ElementScale_3D(nn.Module):
    """A learnable element-wise scaler."""

    def __init__(self, embed_dims, init_value=0., requires_grad=True):
        super(ElementScale_3D, self).__init__()
...
def calculate_model_parameters_in_million(model):
...
class ChannelAggregationFFN_3D(nn.Module):
    ...
    def __init__(self,
                 ...
                 act_type='GELU',
                 ffn_drop=0.):
        super(ChannelAggregationFFN_3D, self).__init__()
...
class MultiOrderDWConv_3D(nn.Module):
    ...
    def __init__(self,
        ...
        super(MultiOrderDWConv_3D, self).__init__()
...
class MultiOrderGatedAggregation_3D(nn.Module):
    ...
    def __init__(self,
        ...
        super(MultiOrderGatedAggregation_3D, self).__init__()
        ...
        self.value = MultiOrderDWConv_3D(
...
class MogaBlock_3D(nn.Module):
    ...
    def __init__(self,
        ...
        super(MogaBlock_3D, self).__init__()
        self.out_channels = embed_dims

        self.norm1 = build_norm_layer(norm_type, embed_dims)

        # spatial attention
        self.attn = MultiOrderGatedAggregation_3D(
            embed_dims,
        ...
        self.mlp = ChannelAggregationFFN_3D(  # DWConv + Channel Aggregation FFN
            embed_dims=embed_dims,
